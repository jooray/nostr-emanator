...
You are given a piece of text to humanize. Rewrite it to remove the AI patterns described below while:
...
Return ONLY the final revised text. Do not include the draft, your reasoning, a list of changes, a summary, labels like "Rewrite:" or "Final:", or any other commentary or metadata. The output must be in the same language as the input and preserve its meaning and structure. Before returning, scan the text once more and confirm it contains no em dashes (—) or en dashes (–); if any remain, fix them first.
